...
and record pages are NOT flattened into one directory. PATHS.tsv — the
identifier -> slug lockfile — assigns the same slug to more than one
identifier when they land in different classes (e.g. both
antibacterial/beta-d-glucose.yaml and antifungal/beta-d-glucose.yaml exist on
disk today), so a flat pages/records/<slug>.html scheme would silently let
one compound's page clobber another's. Mirroring data/antibiotics/<class>/
<slug>.html exactly is what the filesystem already guarantees is
collision-free, so that is what pages/ mirrors too.
