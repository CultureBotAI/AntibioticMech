# YAML Record Review: (S)-temafloxacin

- Repository: CultureBotAI/AntibioticMech
- Record: data/antibiotics/antibacterial/s-temafloxacin.yaml
- Started UTC: 2026-09-24T02:18:29Z
- Finished UTC: 2026-09-24T02:19:15Z
...
| Field | Value |
|---|---|
| Identifier | CHEBI:77795 |
| Label | (S)-temafloxacin |
| Path | data/antibiotics/antibacterial/s-temafloxacin.yaml |
| Class | ANTIBACTERIAL |
| Status | SEEDED |
| Grounding | EXACT |
| Source concept | CHEBI:77795, source version 2026-08-30 |
| Standard InChIKey | QKDHBVNJCZBTMR-NSHDSACASA-N |
...
The target resolves unambiguously to `data/antibiotics/antibacterial/s-temafloxacin.yaml`.
An ignored-file-inclusive search for `CHEBI:77795`, `(S)-temafloxacin`,
`s-temafloxacin`, and `temafloxacin` across `data/antibiotics`, `data/raw`,
`curation`, and `reports/yaml_record_review` found the target record, its
`PATHS.tsv` row, the corresponding `data/raw/chebi_antimicrobials.tsv` and
`curation/record_review_queue.tsv` rows, and the prior `(R)-temafloxacin`
sibling report. A separate ignored-file-inclusive `find` found no exact prior
`reports/yaml_record_review/*s-temafloxacin*` report.

## Validation

| Check | Result |
|---|---|
| `just validate data/antibiotics/antibacterial/s-temafloxacin.yaml` | Passed; `linkml-validate` reported `No issues found`. |
| `just validate-strict data/antibiotics/antibacterial/s-temafloxacin.yaml --out /tmp/s-temafloxacin-strict.tsv` | Passed; 1 file scanned, 0 files with `ERROR`, 0 total `ERROR` rows. |
...
| `just review-queue --limit 160` | Passed; `CHEBI:77795` remains in `review-readiness` because its mechanism is source-seeded and not curator-checked. |
| `just worklist --limit 0 --tsv /tmp/s-temafloxacin-worklist.tsv` | Passed; `CHEBI:77795` appears only in `mechanism` and `review-readiness`. |
...
The ChEBI identity is exact. OLS4 resolves `CHEBI:77795` to `(S)-temafloxacin`
...
`reaxys:4301727` xref carried by the local raw ChEBI row and generated record.
...
- `CHEBI:77795` is the `(S)` enantiomer and has InChIKey
  `QKDHBVNJCZBTMR-NSHDSACASA-N`.
- The local sibling `data/antibiotics/antibacterial/r-temafloxacin.yaml` carries
  `CHEBI:77794` with the mirror-image InChIKey
...
`(R)-temafloxacin` Reaxys xref were inappropriately copied onto the
`(S)`-specific record.

Two ChEBI related synonyms were flattened to exact synonyms. OLS4 reports
`(-)-temafloxacin` and `(S)-(-)-temafloxacin` on `CHEBI:77795` with
`scope: hasRelatedSynonym`, while the local generated record emits the
corresponding minus-sign aliases as `EXACT_SYNONYM`.
...
`CHEBI:77795` carries `CHEBI:50750`, and OLS4 resolves that role to
`EC 5.99.1.3 [DNA topoisomerase (ATP-hydrolysing)] inhibitor`, with DNA gyrase
listed as a related synonym. The generated record's
`NUCLEIC_ACID_SYNTHESIS_INHIBITION` value is therefore an honest restatement of
a ChEBI role, but it remains source-seeded, target-family-level, and not
curator-checked.
...
`(S)-temafloxacin` mechanism:

- Exact PubMed query:
  `QKDHBVNJCZBTMR-NSHDSACASA-N OR CHEBI:77795 OR "(S)-(-)-temafloxacin" OR "(-)-temafloxacin"`.
  PubMed did not match the InChIKey or quoted `(S)-(-)-temafloxacin` phrase and
  broadened the translation to `temafloxacin`.
- Broader PubMed query:
  ...
  fluoroquinolones. Search results are discovery metadata; none was inspected
  as full-text primary support for the exact `(S)` enantiomer.
...
|---|---|---|---|
| Major | Related ChEBI synonyms were emitted as exact synonyms. | OLS4 lists `(-)-temafloxacin` and `(S)-(-)-temafloxacin` on `CHEBI:77795` with `scope: hasRelatedSynonym`, but `s-temafloxacin.yaml` marks the corresponding aliases as `EXACT_SYNONYM`. | Preserve synonym scope in the ChEBI extraction/seeding path, then refresh `data/raw/chebi_antimicrobials.tsv` and re-run `just seed-apply`. The local flattening currently enters through `scripts/seed_from_sources.py`. |
| Major | The source-seeded topoisomerase mechanism has not been reviewed against claim-level primary evidence. | `s-temafloxacin.yaml` has `mode_of_action: NUCLEIC_ACID_SYNTHESIS_INHIBITION` with seeder-owned notes, zero `molecular_targets`, and zero `causal_graphs`; `just worklist --limit 0 --tsv /tmp/s-temafloxacin-worklist.tsv` lists `CHEBI:77795` under `mechanism` with `0 CARD target(s), 0 resistance edge(s) to build on` and under `review-readiness` as source-seeded and not curator-checked. | Add an evidence-backed curator-owned mechanism, molecular target, and causal graph through `write_validated_antibiotic`, or add a curator-owned veto explaining why the mechanism remains unresolved. |
...
1. Preserve ChEBI synonym scope in the import path so `hasRelatedSynonym`
   ...
   `(S)-temafloxacin` record should no longer label related ChEBI synonyms as
   exact synonyms.
2. Inspect `PMID:1846917` in full, and the 15 non-stereospecific PubMed
   ...
   Preserve the distinction between exact `(S)-temafloxacin`, racemic
...
- Re-run `just seed-apply`, then
  `just validate data/antibiotics/antibacterial/s-temafloxacin.yaml` and
  `just verify-corpus --summary`; confirm the record no longer emits ChEBI
  `hasRelatedSynonym` values as `EXACT_SYNONYM`.
- Run the OLS4 exact-term check for `CHEBI:77795` and confirm generated
  synonyms reflect OBO synonym scopes.
- After any mechanism or activity curation, run
  `just validate-strict data/antibiotics/antibacterial/s-temafloxacin.yaml --out /tmp/antibioticmech-record-validation.tsv`,
  `just verify-corpus --summary`,
  `just worklist --limit 0 --tsv /tmp/s-temafloxacin-worklist.tsv`, and
  `just lint`.
...
- The ignored-file-inclusive resolution search covered `data/antibiotics`,
  ...
  `s-temafloxacin`.
- OLS4's `has_role` relation payload for `CHEBI:77795` included `CHEBI:36047`
  and `CHEBI:50750`, plus `CHEBI:35441` as a role ancestor. The local ChEBI
  inventory also carries `CHEBI:33281`, the generic antimicrobial activity role
  retained in the generated record.
- The repository exposes only full-corpus worklist categories for mechanism,
  ...
  `CHEBI:77795` was absent from all inspected queues except `mechanism` and
  `review-readiness`.
