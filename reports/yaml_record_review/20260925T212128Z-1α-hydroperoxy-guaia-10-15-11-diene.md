...
| Live OLS4 `CHEBI:66473`, parent, and role lookups | Passed; `CHEBI:66473` is current and non-obsolete with the same formula, charge, mass values, Standard InChI, InChIKey, SMILES, PubMed xref, Reaxys xref, direct parents `CHEBI:35924` and `CHEBI:36744`, and imported role `CHEBI:36335` used by the generated record. |
...
| ChEBI `CHEBI:66473` | Exact seeded identity and role support. | The live term repeats the same formula, Standard InChI, InChIKey, SMILES, Reaxys xref, PubMed xref, and imported `trypanocidal drug` role as the generated YAML. |
