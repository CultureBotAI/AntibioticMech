# YAML Record Review: (S)-1'-methyloctyl caffeate

- Repository: `CultureBotAI/AntibioticMech`
- Record: `data/antibiotics/antiprotozoal/s-1-methyloctyl-caffeate.yaml`
- Started UTC: `2026-09-23T15:45:11Z`
- Finished UTC: `2026-09-23T15:47:12Z`
...
| Field | Value |
|---|---|
| Identifier | `CHEBI:70484` |
| Label | `(S)-1'-methyloctyl caffeate` |
| Path | `data/antibiotics/antiprotozoal/s-1-methyloctyl-caffeate.yaml` |
| Class | `ANTIPROTOZOAL` |
...
| Source concept | `CHEBI:70484` / `(S)-1'-methyloctyl caffeate`, version `2026-08-30`, role `CHEBI:70868` |
| Standard InChIKey | `DEXGFPWDAXJBTA-WONIAPNHSA-N` |

This review read the entire generated record at
`data/antibiotics/antiprotozoal/s-1-methyloctyl-caffeate.yaml`.

## Validation

| Check | Result |
|---|---|
| `just validate data/antibiotics/antiprotozoal/s-1-methyloctyl-caffeate.yaml` | Passed; LinkML reported `No issues found`. |
| `just validate-strict data/antibiotics/antiprotozoal/s-1-methyloctyl-caffeate.yaml --out /tmp/s-1-methyloctyl-caffeate-strict.tsv` | Passed; 1 file scanned, 0 files with `ERROR`, 0 total `ERROR` rows. |
...
| `just worklist --limit 0 --tsv /tmp/s-1-methyloctyl-caffeate-worklist.tsv` | Passed; `CHEBI:70484` appears in `mechanism`, `producer-candidate`, and `review-readiness`. It is not queued for minted grounding, xref, multi-component, structure, scope, or target-evidence defects. |
| `just review-queue --limit 108` | Passed; `CHEBI:70484` is still queued immediately after the reviewed `(S)-1'-methylbutyl caffeate` row with `MECHANISM_REVIEW: mechanism is absent; 1 source literature lead(s), 0 record evidence item(s), 0 target(s)`. |
| OLS4 ChEBI `CHEBI:70484` term lookup | Passed; the live ChEBI term is active and matched the generated definition, SMILES, Standard InChI, Standard InChIKey, formula, charge, masses, and xref/source lead split. |
| OLS4 ChEBI graph and role lookups | Passed; `CHEBI:65331` is the direct `subClassOf` parent, `CHEBI:78304` is the `has functional parent` nonan-2-ol, `CHEBI:70868` resolves to `antileishmanial agent`, and `CHEBI:76924` is a plant-metabolite role that this antimicrobial corpus correctly does not file as an `activity_roles` value. |
...
| `env -u NCBI_EMAIL uv run python scripts/search_publications.py --provider pubmed --query "\"DEXGFPWDAXJBTA-WONIAPNHSA-N\"[All Fields] OR \"CHEBI:70484\"[All Fields] OR \"(S)-1'-methyloctyl caffeate\"" --limit 20 --output /tmp/s-1-methyloctyl-caffeate-pubmed-exact.jsonl` | PubMed returned two candidates: exact source lead `PMID:20954722`, and `PMID:35305722`, a name-only network-pharmacology COVID-19 hit that does not support this record's antileishmanial claim. |
...
- `data/raw/chebi_antimicrobials.tsv` row 1793 is the maintained ChEBI input
  for `CHEBI:70484`; its label, definition, role, parent, SMILES, Standard
...
- The OLS4 ChEBI `CHEBI:70484` term is not obsolete and has the same Standard
  InChIKey, `DEXGFPWDAXJBTA-WONIAPNHSA-N`.
- ChEBI describes the same `(2S)` nonan-2-yl caffeate ester that the local
  stereospecific SMILES `CCCCCCC[C@H](C)OC(=O)/C=C/c1ccc(O)c(O)c1` encodes.
- The parent `CHEBI:65331` is the structural class `alkyl caffeate ester` and
  correctly belongs in `parent_compounds`.
- OLS4 additionally asserts nonan-2-ol, `CHEBI:78304`, by `has functional
...
|---|---|
| ChEBI identity, definition, synonym, antileishmanial role, parent, Reaxys xref, structure fields, and `PMID:20954722` source reference | Database-seeded from the `CHEBI:70484` row in `data/raw/chebi_antimicrobials.tsv`; no curator-owned literature citation is required at record level. |
...
`1.8 μM`. The abstract does not map those compound numbers to `CHEBI:70484`
...
|---|---|---|
| major | `mode_of_action`, `molecular_targets`, `activity_observations`, and `causal_graphs` are absent even though `PMID:20954722` is an exact ChEBI source lead for antileishmanial activity. The full text must be inspected to map the paper's compound numbers to `CHEBI:70484`, capture any `Leishmania amazonensis` activity measurement, and decide whether the paper supports a specific mode of action or only activity. | Curator-owned additions in `data/antibiotics/antiprotozoal/s-1-methyloctyl-caffeate.yaml`, written through `record_curation_event` and `write_validated_antibiotic`. |
| minor | `Piper sanguineispicum` is present only as a ChEBI isolation statement and a `producer-candidate` worklist row. No `producer_organisms` assertion exists yet, and the source paper should be checked for wording that distinguishes plant biosynthesis from source-material isolation before any producer claim is added. | Curator-owned additions in `data/antibiotics/antiprotozoal/s-1-methyloctyl-caffeate.yaml`, written through `record_curation_event` and `write_validated_antibiotic`. |
...
1. Inspect `PMID:20954722` in full text and map the paper's compound numbers to
   `CHEBI:70484` and its caffeate siblings before adding any activity values.
   Make any update through a guarded mutator that loads
   `data/antibiotics/antiprotozoal/s-1-methyloctyl-caffeate.yaml`, asserts
   `CHEBI:70484`, appends a `codex` `record_curation_event`, and writes with
...
- Rerun
  `just validate data/antibiotics/antiprotozoal/s-1-methyloctyl-caffeate.yaml`
  and `just validate-strict
  data/antibiotics/antiprotozoal/s-1-methyloctyl-caffeate.yaml --out
  /tmp/s-1-methyloctyl-caffeate-strict.tsv`.
...
- Rerun `just worklist --limit 0 --tsv
  /tmp/s-1-methyloctyl-caffeate-worklist.tsv` and verify that `CHEBI:70484`
...
- An ignored/hidden-inclusive search of `reports/yaml_record_review` found no
  prior `s-1-methyloctyl-caffeate` report; it found only the neighboring
  `s-1-methylbutyl-caffeate` review that mentions `CHEBI:70484` as a sibling.
- An ignored/hidden-inclusive search of the target record, `PATHS.tsv`,
  `data/raw`, `curation`, `reports/yaml_record_review`, and
  `/tmp/s-1-methyloctyl-caffeate-worklist.tsv` found `CHEBI:70484` only in the
  generated target, the path lockfile, the ChEBI raw row, the derived
  `record_review_queue.tsv` row, the sibling methylbutyl report, and the
  expected `mechanism`, `producer-candidate`, and `review-readiness` worklist
  rows.
- The same search found no `curation/decisions.tsv` override for
  `antibioticmech:chebi-4fdb2f2426`.
