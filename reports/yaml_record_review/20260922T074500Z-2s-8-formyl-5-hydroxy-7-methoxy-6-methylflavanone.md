# YAML Record Review: (2S)-8-formyl-5-hydroxy-7-methoxy-6-methylflavanone

- Repository: `CultureBotAI/AntibioticMech`
- Record: `data/antibiotics/antiviral/2s-8-formyl-5-hydroxy-7-methoxy-6-methylflavanone.yaml`
- Started UTC: 2026-09-22T07:40:00Z
- Finished UTC: 2026-09-22T07:45:00Z
...
| Field | Value |
|---|---|
| Identifier | `CHEBI:70661` |
| Label | `(2S)-8-formyl-5-hydroxy-7-methoxy-6-methylflavanone` |
| Path | `data/antibiotics/antiviral/2s-8-formyl-5-hydroxy-7-methoxy-6-methylflavanone.yaml` |
| Class | `ANTIVIRAL` |
| Curation status | `SEEDED` |
| Grounding | `EXACT` |
| Source concept | `CHEBI:70661`, version `2026-08-30`, minted key `antibioticmech:chebi-28e22606ff` |
| Source role | `CHEBI:52425` EC 3.2.1.18 inhibitor |
| Structure | `QJQJKEWQUOORFC-AWEZNQCLSA-N`; formula `C18H16O5`; charge `0` |
| Same-structure xrefs | `reaxys:10367824` |
...
| Check | Result |
|---|---|
| `just validate data/antibiotics/antiviral/2s-8-formyl-5-hydroxy-7-methoxy-6-methylflavanone.yaml` | Pass: `linkml-validate` reported `No issues found`. |
| `just validate-strict data/antibiotics/antiviral/2s-8-formyl-5-hydroxy-7-methoxy-6-methylflavanone.yaml --out /tmp/antibioticmech-chebi-70661-validation.tsv` | Pass: 1 file scanned, 0 files with `ERROR`, 0 total error rows. |
...
| `just worklist --limit 0 --tsv /tmp/antibioticmech-worklist-chebi-70661.tsv` | Pass: the full worklist TSV was written. `CHEBI:70661` appears on `mechanism`, `producer-candidate`, and `review-readiness`; it was not found on any other queue in that TSV. |
| `just review-queue --limit 50` | Pass: `CHEBI:70661` is queued with `MECHANISM_REVIEW: mechanism is source-seeded and not curator-checked; 1 source literature lead(s), 0 record evidence item(s), 0 target(s)`. |
...
`CHEBI:70661` resolves in official OLS as a current, non-obsolete ChEBI term
labeled `(2S)-8-formyl-5-hydroxy-7-methoxy-6-methylflavanone`. The live OLS
...
official OLS hierarchical parents of `CHEBI:70661`: `CHEBI:17478` aldehyde,
...
`reaxys:10367824` and `pubmed:20886838` to this exact term; the YAML imports the
...
An ignored-inclusive search for `CHEBI:70661`, the exact label, and the
...
target, resistance, producer, or causal-graph claim resolving `CHEBI:70661`.
...
The committed raw ChEBI row for `CHEBI:70661` has one source-literature lead:
...
does not map exact flavanone `CHEBI:70661` to a source-paper compound number or
...
`QJQJKEWQUOORFC-AWEZNQCLSA-N` found zero PubMed candidates. A second search for
...
`CHEBI:70661` was one of the source paper's neuraminidase-inhibiting compounds,
...
|---|---|---|---|
| Major | The source-seeded neuraminidase-inhibition mechanism has not been curator-checked against claim-level primary evidence for exact `CHEBI:70661`. | `mode_of_action` and `mode_of_action_target_scope` were seeded from `CHEBI:52425`; the record has zero record evidence items and zero targets; the only ChEBI source-literature lead is `PMID:20886838`; PubMed's abstract names the active chalcones but does not map exact flavanone `CHEBI:70661` to a compound number, IC50, enzyme target, or influenza strain. | Future curator-owned fields on `data/antibiotics/antiviral/2s-8-formyl-5-hydroxy-7-methoxy-6-methylflavanone.yaml`, written through `record_curation_event` and `write_validated_antibiotic`; if ChEBI later changes this term's role or source xrefs, the ChEBI inventory extractor owns that refresh. |
...
3. Add `activity_spectrum` rows only for source-reported observations with exact
   ...
   fields only if the table maps quantitative values to `CHEBI:70661`.
...
1. `just validate data/antibiotics/antiviral/2s-8-formyl-5-hydroxy-7-methoxy-6-methylflavanone.yaml`
2. `just validate-strict data/antibiotics/antiviral/2s-8-formyl-5-hydroxy-7-methoxy-6-methylflavanone.yaml --out /tmp/antibioticmech-chebi-70661-validation.tsv`
3. `just verify-corpus --summary`
4. `just worklist --limit 0 --tsv /tmp/antibioticmech-worklist-chebi-70661.tsv`
5. `just review-queue --limit 50`
6. `just qc`
...
neuraminidase-inhibition claim is attached to exact `CHEBI:70661` rather than a
