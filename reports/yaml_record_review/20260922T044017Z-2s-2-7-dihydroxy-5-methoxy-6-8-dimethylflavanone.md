# YAML Record Review: (2S)-2,7-dihydroxy-5-methoxy-6,8-dimethylflavanone

- Repository: `CultureBotAI/AntibioticMech`
- Record: `data/antibiotics/antiviral/2s-2-7-dihydroxy-5-methoxy-6-8-dimethylflavanone.yaml`
- Started UTC: 2026-09-22T04:34:00Z
- Finished UTC: 2026-09-22T04:40:17Z
...
| Field | Value |
|---|---|
| Identifier | `CHEBI:70654` |
| Label | `(2S)-2,7-dihydroxy-5-methoxy-6,8-dimethylflavanone` |
| Path | `data/antibiotics/antiviral/2s-2-7-dihydroxy-5-methoxy-6-8-dimethylflavanone.yaml` |
| Class | `ANTIVIRAL` |
| Curation status | `SEEDED` |
| Grounding | `EXACT` |
| Source concept | `CHEBI:70654`, version `2026-08-30`, minted key `antibioticmech:chebi-d262a1595d` |
| Source role | `CHEBI:52425` EC 3.2.1.18 inhibitor |
| Structure | `BIMVXOYXXZYVJF-SFHVURJKSA-N`; formula `C18H18O5`; charge `0` |
| Same-structure xrefs | `reaxys:21040402` |
...
source-derived identity, definition, one exact IUPAC synonym, three ChEBI
parents, one exo-alpha-sialidase-inhibitor role, one source-seeded
`VIRAL_RELEASE_INHIBITION` mode, one same-structure Reaxys xref, structure, and
...
| Check | Result |
|---|---|
| `just validate data/antibiotics/antiviral/2s-2-7-dihydroxy-5-methoxy-6-8-dimethylflavanone.yaml` | Pass: `linkml-validate` reported `No issues found`. |
| `just validate-strict data/antibiotics/antiviral/2s-2-7-dihydroxy-5-methoxy-6-8-dimethylflavanone.yaml --out /tmp/antibioticmech-chebi-70654-validation.tsv` | Pass: 1 file scanned, 0 files with `ERROR`, 0 total error rows. |
...
| `just worklist --limit 0 --tsv /tmp/antibioticmech-worklist-chebi-70654.tsv` | Pass: the full worklist TSV was written. `CHEBI:70654` appears on `mechanism`, `producer-candidate`, and `review-readiness`; it was not found on any other queue in that TSV. |
| `just review-queue --limit 47` | Pass: `CHEBI:70654` is the first queued record after the records that already have `reports/yaml_record_review` reports; its row says `MECHANISM_REVIEW: mechanism is source-seeded and not curator-checked; 1 source literature lead(s), 0 record evidence item(s), 0 target(s)`. |
...
`CHEBI:70654` resolves in official OLS as a current, non-obsolete ChEBI term
labeled `(2S)-2,7-dihydroxy-5-methoxy-6,8-dimethylflavanone`. The live OLS
...
The generated record's three `parent_compounds` entries are exactly the official
OLS hierarchical parents of `CHEBI:70654`: `CHEBI:141992`
2-hydroxyflavanones, `CHEBI:38738` monomethoxyflavanone, and `CHEBI:38749`
dihydroxyflavanone. The official OLS graph also links the reviewed term to
`CHEBI:15606` `(2S)-flavanone` by `has functional parent` and to `CHEBI:76924`
plant metabolite by `has role`; those broader edges do not belong in
`parent_compounds` or antimicrobial `activity_roles`.
...
`reaxys:21040402` and `pubmed:20886838` to this exact term; the YAML imports the
...
An ignored-inclusive search for `CHEBI:70654`,
`BIMVXOYXXZYVJF-SFHVURJKSA-N`, the exact label, and the filename stem across
`data/antibiotics`, `data/raw`, `curation`, `reports`, `.claude`, `docs`, and
`README.md` found only the expected PATHS row, generated YAML, raw ChEBI row,
and review-readiness row. It found no prior dedicated review report, curation
decision, curator-inventory row, generated activity observation, molecular
target, resistance, producer, or causal-graph claim resolving `CHEBI:70654`.
...
The committed raw ChEBI row for `CHEBI:70654` has one source-literature lead:
...
and 14, all with a chalcone skeleton, had significant inhibitory effects on
H1N1 and H9N2 viral neuraminidases; compound 4 was strongest against wild-type
and H274Y-mutant novel H1N1 neuraminidases expressed in 293T cells. The
abstract is enough to confirm the ChEBI source paper's antiviral screening
context, but it does not map exact `CHEBI:70654` to a source-paper compound
number or expose an IC50 for this structure.

The plant name from the definition did not resolve as a current NCBI Taxonomy
scientific name by the query `Cleistocalyx operculatus[SCIN]`, but the NCBI
all-names search maps `Cleistocalyx operculatus` to species `NCBITaxon:219895`,
whose current scientific name is `Syzygium nervosum` and whose synonym list
retains `Cleistocalyx operculatus`.

A bounded repository publication search for the exact Standard InChIKey
`BIMVXOYXXZYVJF-SFHVURJKSA-N` found zero PubMed candidates. A second search for
...
exact ChEBI identity, structure, chemically broader flavanone parents, an
exo-alpha-sialidase-inhibitor role, a source-seeded release-inhibition mode, one
same-structure registry xref, and a primary-paper lead, but no curator-owned
...
`CHEBI:70654` was one of the source paper's tested compounds, whether the
reported target was a specific influenza neuraminidase, whether a quantitative
IC50 or only a qualitative observation is in scope, and whether any viral strain
or H274Y-mutant context can be curated.
...
|---|---|---|---|
| Major | The source-seeded neuraminidase-inhibition mechanism has not been curator-checked against claim-level primary evidence for exact `CHEBI:70654`. | `mode_of_action` and `mode_of_action_target_scope` were seeded from `CHEBI:52425`; the record has zero record evidence items and zero targets; the only ChEBI source-literature lead is `PMID:20886838`; PubMed's abstract does not map exact `CHEBI:70654` to a compound number, IC50, enzyme target, or influenza strain. | Future curator-owned fields on `data/antibiotics/antiviral/2s-2-7-dihydroxy-5-methoxy-6-8-dimethylflavanone.yaml`, written through `record_curation_event` and `write_validated_antibiotic`; if ChEBI later changes this term's role or source xrefs, the ChEBI inventory extractor owns that refresh. |
...
3. Add `activity_spectrum` rows only for source-reported observations with exact
   ...
   only if the table maps quantitative values to `CHEBI:70654`.
...
1. `just validate data/antibiotics/antiviral/2s-2-7-dihydroxy-5-methoxy-6-8-dimethylflavanone.yaml`
2. `just validate-strict data/antibiotics/antiviral/2s-2-7-dihydroxy-5-methoxy-6-8-dimethylflavanone.yaml --out /tmp/antibioticmech-chebi-70654-validation.tsv`
3. `just verify-corpus --summary`
4. `just worklist --limit 0 --tsv /tmp/antibioticmech-worklist-chebi-70654.tsv`
5. `just review-queue --limit 47`
6. `just qc`
...
neuraminidase inhibition claim is attached to exact `CHEBI:70654` rather than a
