...
from antibioticmech.text_map_inputs import export_inputs
...
def load_pipeline(root: Path) -> ModuleType:
    ...
    spec = importlib.util.spec_from_file_location("antibioticmech_embedding_pipeline", path)
...
def prepare_text_map(root: Path) -> Iterator[PreparedTextMap | None]:
    ...
    with tempfile.TemporaryDirectory(prefix="antibioticmech-text-map-") as directory:
